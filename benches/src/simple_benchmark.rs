...
fn main() -> Result<()> {
    ...
    for (i, image_path) in test_images.iter().enumerate() {
        ...
        println!("   JPEG XS: {:.1}:1 compression, {:.2}ms encode, {:.1} MB/s", 
            r.jpegxs_compression_ratio, r.jpegxs_encode_time_ms, r.jpegxs_encode_throughput_mbps);
        println!("   JPEG:    {:.1}:1 compression, {:.2}ms encode", 
...
fn benchmark_jpegxs(
    ...
    let avg_encode_ms = encode_times.iter().sum::<Duration>().as_secs_f64() / iterations as f64 * 1000.0;
    let avg_decode_ms = decode_times.iter().sum::<Duration>().as_secs_f64() / iterations as f64 * 1000.0;
...
fn print_summary(results: &[BenchmarkResult]) {
    ...
    let avg_jpegxs_encode: f64 = results.iter().map(|r| r.jpegxs_encode_time_ms).sum::<f64>() / results.len() as f64;
    let avg_jpeg_encode: f64 = results.iter().map(|r| r.jpeg_encode_time_ms).sum::<f64>() / results.len() as f64;
    let avg_png_encode: f64 = results.iter().map(|r| r.png_encode_time_ms).sum::<f64>() / results.len() as f64;

    let avg_jpegxs_throughput: f64 = results.iter().map(|r| r.jpegxs_encode_throughput_mbps).sum::<f64>() / results.len() as f64;

    println!("\n🗜️  COMPRESSION COMPARISON:");
    println!("   JPEG XS:  {:.1}:1 ratio ({:.1} KB total)", total_original_size / total_jpegxs_size, total_jpegxs_size);
    println!("   JPEG:     {:.1}:1 ratio ({:.1} KB total)", total_original_size / total_jpeg_size, total_jpeg_size);
    println!("   PNG:      {:.1}:1 ratio ({:.1} KB total)", total_original_size / total_png_size, total_png_size);

    println!("\n⚡ SPEED COMPARISON:");
    println!("   JPEG XS:  {:.2} ms avg encode ({:.1} MB/s)", avg_jpegxs_encode, avg_jpegxs_throughput);
    println!("   JPEG:     {:.2} ms avg encode", avg_jpeg_encode);
    println!("   PNG:      {:.2} ms avg encode", avg_png_encode);

    // Determine winner categories
    let best_compression = if total_original_size / total_jpegxs_size > total_original_size / total_jpeg_size {
        "JPEG XS"
    } else {
        "JPEG"
    };
    
    let fastest_encode = if avg_jpegxs_encode < avg_jpeg_encode && avg_jpegxs_encode < avg_png_encode {
        "JPEG XS"
    ...
    if best_compression == "JPEG XS" {
        let improvement = ((total_original_size / total_jpegxs_size) / (total_original_size / total_jpeg_size) - 1.0) * 100.0;
        println!("   🎉 JPEG XS achieves {:.1}% better compression than JPEG!", improvement);
    }
}
...
fn generate_markdown_report(results: &[BenchmarkResult]) -> Result<String> {
    ...
    md.push_str(&format!("- **JPEG XS Compression**: {:.1}:1 ratio\n", total_original_size / total_jpegxs_size));
    md.push_str(&format!("- **JPEG Compression**: {:.1}:1 ratio\n", total_original_size / total_jpeg_size));
    
    let improvement = ((total_original_size / total_jpegxs_size) / (total_original_size / total_jpeg_size) - 1.0) * 100.0;
    if improvement > 0.0 {
        md.push_str(&format!("- **JPEG XS vs JPEG**: {:.1}% better compression\n", improvement));
    }
