...
mod tests {
    ...
    fn test_dwt_53_roundtrip_1d() {
        ...
        for (orig, reconstructed) in original.iter().zip(data.iter()) {
            assert!((orig - reconstructed).abs() < 1e-6, 
                    "Roundtrip error too large: {} vs {}", orig, reconstructed);
        }
    }
    ...
    fn test_dwt_53_roundtrip_2d() {
        ...
        for (orig, reconstructed) in original.iter().zip(data.iter()) {
            assert!((orig - reconstructed).abs() < 1e-6,
                    "2D Roundtrip error too large: {} vs {}", orig, reconstructed);
        }
    }
    ...
    fn test_dwt_53_properties() {
        ...
        assert!(transformed_energy > 0.0, "Transform should produce non-zero output");
        assert!(transformed_energy != original_energy, "Transform should change energy distribution");
        
        // Verify we have reasonable energy distribution
        assert!(transformed_energy > original_energy * 0.3, 
                "Transform shouldn't lose too much energy: {} vs {}", original_energy, transformed_energy);
    }
