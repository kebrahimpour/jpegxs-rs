...
pub fn vlc_decode(reader: &mut BitstreamReader, ctx: VlcContext) -> Result<i32> {
    ...
    let mut n = 0i32;
    let max_consecutive = if br == 4 { 32 } else if br == 5 { 64 } else { 1 << (br + 1) };
    ...
    loop {
        ...
        if n >= max_consecutive {
            return Err(anyhow::anyhow!("VLC decoder error: too many consecutive 1-bits"));
        }
    }
...
mod tests {
    ...
    fn test_vlc_roundtrip() {
        ...
        let encoded = writer.finalize();
        println!("Encoded {} values into {} bytes", test_values.len(), encoded.len());
