...
impl ReferenceImplementation for IsoReference {
    ...
    fn encode(&self, input: &ImageOwned8, quality: f32) -> Result<Vec<u8>> {
        ...
        let output = Command::new(&self.executable_path)
            ...
            .arg(&output_path)
            .arg("-w")
            .arg(input.width.to_string())
            .arg("-h")
            .arg(input.height.to_string())
            .arg("-q")
    ...
    fn decode(&self, bitstream: &[u8]) -> Result<ImageOwned8> {
        ...
        if !output.status.success() {
            return Err(anyhow::anyhow!(
                "Reference decoder failed: {}",
                String::from_utf8_lossy(&output.stderr)
            ));
        }

        let data = std::fs::read(&output_path).context("Failed to read decoded output")?;
...
pub fn compare_with_reference(
    ...
    let our_start = Instant::now();
    let our_decoded =
        jpegxs_core::decode_frame(&our_encoded, &jpegxs_core::DecoderConfig::default())?;
...
pub fn generate_comparison_report(results: &[ComparisonResult]) -> String {
    ...
    let avg_encode_ratio =
        results.iter().map(|r| r.encode_time_ratio).sum::<f64>() / results.len() as f64;
    let avg_decode_ratio =
        results.iter().map(|r| r.decode_time_ratio).sum::<f64>() / results.len() as f64;
    ...
    report.push_str(&format!(
        "- **Average PSNR Difference**: {:+.2} dB\n",
        avg_psnr_diff
    ));
    report.push_str(&format!(
        "- **Average Encode Speed**: {:.2}x {}\n",
        avg_encode_ratio,
        if avg_encode_ratio < 1.0 {
            "faster"
        } else {
            "slower"
        }
    ...
    report.push_str(&format!(
        "- **Average Decode Speed**: {:.2}x {}\n",
        avg_decode_ratio,
        if avg_decode_ratio < 1.0 {
            "faster"
        } else {
            "slower"
        }
